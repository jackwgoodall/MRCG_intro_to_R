...
# Layers get progressively added to a base create then 'finesse' your plot 
...
# load it if needed (uncomment these lines to run them): 
library(tidyverse)
...
ggplot(data = iris_df) # This won't give an error but it will just produce a blank chart

# We can then add our chart type and define our x and y axes
ggplot(data = iris_df) + # the plus has to be on the same line
...
# Species is a character variable so we can use this to *group*
str(iris_df$Species)
...
# This can also be with a continuous variable
# eg
...
# We can also change the shape (this must be a categorical variable)
...
my_plot + # You could type out the whole plot above - this is just a way to keep things neat
 ...
 my_plot + 
   ...
   theme(plot.title = element_text(size = 16, 
         ...
         axis.ticks = element_line(colour = "cadetblue",
                                   linewidth = 0.5),
         ...
         panel.grid.major.y = element_line(colour = "cadetblue", 
                                           size = 0.3),
